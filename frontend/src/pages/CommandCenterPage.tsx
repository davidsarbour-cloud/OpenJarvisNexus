...
import { LiveSatellite } from '../components/CommandCenter/FunctionalSatellites';

/**
 * CommandCenterPage — route `/`.
 *
 * After the Phase 7 card migration, every service card was relocated to
 * its themed `/world/<name>` room. Only globally-scoped views remain here:
 *   - DAILY DIGEST — chiffres du jour (Forge, Vault, errors)
 *   - AUTOMATION SCHEDULE — every APScheduler job across all rooms
 * Navigation to worlds lives in the left sidebar (HudSidebar).
 */
export function CommandCenterPage() {
  return (
    <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-4">
      <SectionTitle text="GLOBAL · TODAY" />
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 xl:grid-cols-6 2xl:grid-cols-10 gap-3">
        <div className="col-span-2 sm:col-span-3 lg:col-span-5 xl:col-span-6 2xl:col-span-10">
          <LiveSatellite title="DAILY DIGEST" colorKey="jarvis" snapshotKey="daily_digest" />
        </div>
      </div>
