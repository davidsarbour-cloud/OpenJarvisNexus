...
# Split-out task groups (kept importable from this module so
# create_scheduler() and daily_router still resolve them by name).
from daily_tasks_common import _write_skill_brain_note  # noqa: E402,F401
from daily_tasks_reports import (  # noqa: E402
    task_daily_brain_stubs_check,
    task_monthly_brain_snapshot,
    task_monthly_repo_audit,
    task_weekly_vault_growth,
)
...
def create_scheduler():
    ...
    if _af_cfg.get("enabled", False):
        ...
        if "uikit" in _af_products:
            scheduler.add_job(
                task_auto_factory_uikit,
                trigger=CronTrigger(hour=_uk_h, minute=_uk_m),
                id="auto_factory_uikit",
                name=f"Daily: UI Kit Factory ({_uk_h:02d}:{_uk_m:02d})",
                replace_existing=True,
                misfire_grace_time=3600,
            )
